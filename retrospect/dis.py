...
if type(bytes('0'.encode('utf8'))[0]) != int:
    class bytes(bytes):
        def __getitem__(self, key):
            val = super(bytes, self).__getitem__(key)
...
class Instruction(_Instruction):
    ...
    def codestring(self):
        ...
        codestring = chr(self.opcode)
        if self.arg is not None:
            codestring += chr(self.arg % 256) + chr(self.arg // 256)
        return codestring
...
def _get_instructions_bytes(code, varnames=None, names=None, constants=None,
    ...
    while i < n:
        ...
        ins = Instruction(opname[op], op,
                          arg, argval, argrepr,
                          offset, starts_line, is_jump_target)
        #FIXME: UPSTREAM?
        ins.code = code[offset:i]
        yield ins
